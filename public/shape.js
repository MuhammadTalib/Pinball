class Shape{
    ...
    drawFromLines(){
        for(var i=0;i<this.lines.length;i++){
            ctx.moveTo(this.lines[i].p1.x,this.lines[i].p1.y);
    ...
    touchedShape(p){
        for(var i=0;i<this.lines.length;i++){
            var a=this.touchedLine(this.lines[i],p)
            if(a.state){
                //console.log
                return a
            }
    ...
    touchedLine(l,p){
        var LHS=(p.y+21-l.p1.y)*(l.p2.x-l.p1.x)
        var RHS=(l.p2.y-l.p1.y)*(p.x+21-l.p1.x)
        
        if(LHS===RHS){
            return {val:0,state:true}
        }
        else if(LHS>RHS){
            return {val:-1,state:false}
        }
        return {val:1,state:false}
    }
    findPoint(p){
